...
def streamlit_cloud_runtime_enabled():
    if cloud_safe_mode_enabled():
        return True

    cloud_env_names = ("STREAMLIT_CLOUD", "STREAMLIT_SHARING_MODE", "STREAMLIT_RUNTIME_ENV")
    for name in cloud_env_names:
        value = str(os.environ.get(name, "")).strip().lower()
        if value in ("1", "true", "yes", "on", "cloud", "streamlit", "streamlit-cloud"):
            return True
    return False


SCAN_MODE_STABLE = "stable"
SCAN_MODE_FULL = "full"
SCAN_MODE_LIMITS = {
    SCAN_MODE_STABLE: 100,
    SCAN_MODE_FULL: 300,
}
SCAN_MODE_LABELS = {
    SCAN_MODE_STABLE: "穩定模式 100",
    SCAN_MODE_FULL: "完整模式 300",
}


def default_scan_mode():
    return SCAN_MODE_STABLE if streamlit_cloud_runtime_enabled() else SCAN_MODE_FULL


def scan_mode_limit(scan_mode):
    return SCAN_MODE_LIMITS.get(scan_mode, SCAN_MODE_LIMITS[default_scan_mode()])


def scan_mode_label(scan_mode):
    return SCAN_MODE_LABELS.get(scan_mode, SCAN_MODE_LABELS[default_scan_mode()])
...
def run_scan(scan_limit):
    cloud_safe_mode = streamlit_cloud_runtime_enabled()
...
def show_manual_refresh_summary(summary):
    if not summary:
        return

    st.success("手動刷新完成")
    st.info(f"本次掃描模式：{summary.get('scan_mode')}")
    st.info(f"本次開始時間：{summary.get('start_time')}")
...
def perform_manual_refresh(scan_limit, scan_mode=None):
    ...
    try:
        ...
        refresh_finish_time = format_taipei_dt()
        refresh_summary = build_manual_refresh_summary(
            scan_mode,
            scan_limit,
            refresh_start_time,
            refresh_finish_time,
        ...
        return payload, trade_tracking, line_status_text, github_persist_result, refresh_summary
    except Exception as exc:
...
with st.sidebar:
    ...
    scan_mode = default_scan_mode()
    scan_limit = scan_mode_limit(scan_mode)
    ...
    if st.session_state.get("admin_ok") == True:
        scan_mode_options = [SCAN_MODE_STABLE, SCAN_MODE_FULL]
        scan_mode = st.selectbox(
            "Cloud 掃描模式",
            options=scan_mode_options,
            format_func=scan_mode_label,
            index=scan_mode_options.index(default_scan_mode()),
        )
        scan_limit = scan_mode_limit(scan_mode)

        show_manual_refresh_summary(st.session_state.get("manual_refresh_summary"))

        if st.button("🔄 手動刷新今日資料"):
            with st.spinner("正在手動刷新今日資料，請勿關閉頁面。"):
                _, _, refresh_line_status, github_persist_result, refresh_summary = perform_manual_refresh(scan_limit, scan_mode)
...
if df.empty:
    if analysis_result_error:
        ...
        st.info("目前沒有符合資料。可切換策略，或請管理員手動刷新。")
else:
